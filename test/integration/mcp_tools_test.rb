...
class McpToolsTest < ActionDispatch::IntegrationTest
  ...
  test "every listed tool but the write tools declares itself read-only and non-destructive" do
    ...
    assert_equal %w[create_folder get_attachment get_mail_account get_message list_folders list_mail_accounts move_message search_contacts search_messages set_flags trash_message],
      listed.map { |tool| tool["name"] }
    tools = listed.reject { |tool| %w[create_folder move_message set_flags trash_message].include?(tool["name"]) }
    tools.each do |tool|
