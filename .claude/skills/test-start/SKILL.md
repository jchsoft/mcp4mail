...
- `command` — the full test command, already resolved by `/test-runner` from the project’s
  `.claude/test-commands.json` (or from its detection table when that file is absent). Passed
  through verbatim; this skill neither parses it nor knows what runner it names.
